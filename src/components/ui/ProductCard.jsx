  ...
  return (
    <div className="group relative bg-white border border-slate-200 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
      ...
      <div className="p-6 relative">
        ...
        <Link to={`/produit/${product.id}`}>
          <h3 className="text-lg font-bold text-secondary-dark line-clamp-1 hover:text-primary transition-colors relative z-10">
            {product.name}
          </h3>
        </Link>
        <p className="mt-2 text-xs text-secondary line-clamp-2 font-light relative z-10">
          {product.description}
        </p>
